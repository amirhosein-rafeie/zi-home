...
function Card({ item }) {
  return (
    <div className="bg-white w-full max-w-[288px] mx-auto border-[1px] border-[#D1D4D4] rounded-2xl shadow flex justify-center items-center flex-col pt-6 pr-4 pb-6 pl-4">
      <img
        src={item.image}
        alt={item.title}
      ...
      <div className="gap-6 w-full h-40 grid  grid-rows-2 grid-cols-1">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-2 lg:gap-3 xl:gap-5">
          ...
          <div>
            <p className="bg-[#F87171] w-14 h-8 rounded-2xl border-[#FDF2F6] border-2 text-[#FFFFFF] pt-1 pb-1 pr-2.5 pl-2.5">
              {item.off}
            </p>
          </div>
        </div>
